...
#include <QFileDialog>
OtherStockSelect::OtherStockSelect(QWidget *parent)
    : QMainWindow(parent)
{
...
void OtherStockSelect::createButton()
{
...
void OtherStockSelect::createTableView()
{
...
void OtherStockSelect::createLabel_LineEdit()
{
...
void OtherStockSelect::createList()
{
    selectList.clear();
...
void OtherStockSelect::paintEvent(QPaintEvent*)
...
void OtherStockSelect::showStatus()
{
    exportthread->quit();
    delete exportthread;
    exportthread=NULL;
...
void OtherStockSelect::exportFun()
{
    ...
    else
    {
        ...
        exportthread=new ExportExcelThread(this,fileName,otherstockList,row,cloumn);
        exportthread->start();
        connect(exportthread,SIGNAL(sendStatus()),this,SLOT(showStatus()));
    }
}
/*void OtherStockSelect::printFun()
{

}*/

void OtherStockSelect::clearTableView()
{
...
void OtherStockSelect::selectFun()
{
    createList();
    clearTableView();
    QSqlQuery query;
    ...
    case 0:
    {
        ...
        select_sql = "select   goodsinput.goodsid,goodsinput.goodsname,goodsinput.inputcount ,goods.goodsunit,\
                ...
                (  select id from goodsinput where inputtime>=:conditionStr1 and inputtime<=:conditionStr2) \
                 and goods.goodsid=goodsinput.goodsid and goodsinput.inputtype='其他入库'";
    ...
    else
    {
        ...
        otherstockList.clear();
        otherstockList.append("产品编号");
        otherstockList.append("产品名称");
        otherstockList.append("入库数量");
        otherstockList.append("计量单位");
        otherstockList.append("入库单价");
        otherstockList.append("入库总价");
        otherstockList.append("入库类型");
        otherstockList.append("安全库存");
        otherstockList.append("产品状态");
        otherstockList.append("备注");
        otherstockList.append("入库时间");
        while(query.next())
        {
            tableModel->setItem(i,0,new QStandardItem(query.value("goodsid").toString()));
            otherstockList.append(query.value("goodsid").toString());

            tableModel->setItem(i,1,new QStandardItem(query.value("goodsname").toString()));
            otherstockList.append(query.value("goodsname").toString());

            tableModel->setItem(i,2,new QStandardItem(query.value("inputcount").toString()));
            otherstockList.append(query.value("inputcount").toString());


            tableModel->setItem(i,3,new QStandardItem(query.value("goodsunit").toString()));
            otherstockList.append(query.value("goodsunit").toString());


            tableModel->setItem(i,4,new QStandardItem(query.value("inputprice").toString()));
            otherstockList.append(query.value("inputprice").toString());


            tableModel->setItem(i,5,new QStandardItem(query.value("inputtotal").toString()));
            otherstockList.append(query.value("inputtotal").toString());


            tableModel->setItem(i,6,new QStandardItem(query.value("inputtype").toString()));
            otherstockList.append(query.value("inputtype").toString());


            tableModel->setItem(i,7,new QStandardItem(query.value("goodssave").toString()));
            otherstockList.append(query.value("goodssave").toString());


            tableModel->setItem(i,8,new QStandardItem(query.value("goodsstatus").toString()));
            otherstockList.append(query.value("goodsstatus").toString());
            ...
            otherstockList.append(query.value("goodsremarks").toString());



            tableModel->setItem(i,10,new QStandardItem(query.value("inputtime").toDate().toString("yyyy-MM-dd")));
            otherstockList.append(query.value("inputtime").toDate().toString("yyyy-MM-dd"));
