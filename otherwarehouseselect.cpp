...
OtherWarehouseSelect::OtherWarehouseSelect(QWidget *parent)
    : QMainWindow(parent)
{
...
void OtherWarehouseSelect::createButton()
{
...
void OtherWarehouseSelect::createList()
{
    selectList.clear();
...
void OtherWarehouseSelect::createTableView()
{
...
void OtherWarehouseSelect::createLabel_LineEdit()
{
...
void OtherWarehouseSelect::paintEvent(QPaintEvent*)
...
void OtherWarehouseSelect::showStatus()
{
    exportthread->quit();
    delete exportthread;
    exportthread=NULL;
...
void OtherWarehouseSelect::exportFun()
{
    ...
    else
    {
        ...
        exportthread=new ExportExcelThread(this,fileName,otherwareList,row,cloumn);
        exportthread->start();
        connect(exportthread,SIGNAL(sendStatus()),this,SLOT(showStatus()));
    }
}
void OtherWarehouseSelect::printFun()
{

}


void OtherWarehouseSelect::clearTableView()
{
...
void OtherWarehouseSelect::selectFun()
{
    createList();
    clearTableView();
    QSqlQuery query;
    ...
    case 0:
    {
        ...
        select_sql = "select   goodsoutput.goodsid,goodsoutput.goodsname,goodsoutput.outputcount ,goods.goodsunit,\
                ...
                (  select id from goodsoutput where outputtime>=:conditionStr1 and outputtime<=:conditionStr2) \
                 and goods.goodsid=goodsoutput.goodsid and goodsoutput.outputtype='其他出库'";
    ...
    case 1:
    {
        ...
        select_sql = "select   goodsoutput.goodsid,goodsoutput.goodsname,goodsoutput.outputcount ,goods.goodsunit,\
                ...
                goods,goodsoutput where goodsoutput.goodsname in(select goodsname from goodsoutput \
                    ...
                    and goodsoutput.outputtype='其他出库'";
    ...
    else
    {
        ...
        otherwareList.clear();
        otherwareList.append("产品编号");
        otherwareList.append("产品名称");
        otherwareList.append("出库数量");
        otherwareList.append("计量单位");
        otherwareList.append("出库单价");
        otherwareList.append("出库总价");
        otherwareList.append("出库类型");
        otherwareList.append("安全库存");
        otherwareList.append("产品状态");
        otherwareList.append("备注");
        otherwareList.append("出库时间");
        while(query.next())
        {
            tableModel->setItem(i,0,new QStandardItem(query.value("goodsid").toString()));
            otherwareList.append(query.value("goodsid").toString());

            tableModel->setItem(i,1,new QStandardItem(query.value("goodsname").toString()));
            otherwareList.append(query.value("goodsname").toString());

            tableModel->setItem(i,2,new QStandardItem(query.value("outputcount").toString()));
            otherwareList.append(query.value("outputcount").toString());


            tableModel->setItem(i,3,new QStandardItem(query.value("goodsunit").toString()));
            otherwareList.append(query.value("goodsunit").toString());


            tableModel->setItem(i,4,new QStandardItem(query.value("outputprice").toString()));
            otherwareList.append(query.value("outputprice").toString());


            tableModel->setItem(i,5,new QStandardItem(query.value("outputtotal").toString()));
            otherwareList.append(query.value("outputtotal").toString());


            tableModel->setItem(i,6,new QStandardItem(query.value("outputtype").toString()));
            otherwareList.append(query.value("outputtype").toString());


            tableModel->setItem(i,7,new QStandardItem(query.value("goodssave").toString()));
            otherwareList.append(query.value("goodssave").toString());


            tableModel->setItem(i,8,new QStandardItem(query.value("goodsstatus").toString()));
            otherwareList.append(query.value("goodsstatus").toString());
            ...
            otherwareList.append(query.value("goodsremarks").toString());



            tableModel->setItem(i,10,new QStandardItem(query.value("outputtime").toDate().toString("yyyy-MM-dd")));
            otherwareList.append(query.value("outputtime").toDate().toString("yyyy-MM-dd"));
